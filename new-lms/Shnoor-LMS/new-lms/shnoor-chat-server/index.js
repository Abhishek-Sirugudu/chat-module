...
// Configure Multer (Store files in memory buffer before saving to DB)
const upload = multer({
...
const runMigrations = async () => {
  try {
    ...
    // 2. Add columns to messages table if they don't exist
...
// 6. Get Messages (Joined with Files)
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async (data) => {
    ...
    try {
      ...
      // Fetch file details if needed for immediate display
      let fileData = {};
      ...
      const messagePayload = {
        ...savedMsg.rows[0],
        ...fileData, // Spread file metadata for frontend
        chat_id,
        sender_uid: sender_firebase_uid,
        receiver_uid: receiver_firebase_uid,
        client_side_id: data.client_side_id // Pass back the client-side ID
      };
