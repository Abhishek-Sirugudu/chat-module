...
const StudentChat = () => {
    ...
    // Keep Refs synced for socket listener
    ...
    // Socket Listener
    // Find this useEffect in InstructorChat.jsx and replace
    useEffect(() => {
        if (!socket) return;
        ...
        const handleReceiveMessage = (newMsg) => {
            ...
            if (currentActive && String(newMsg.chat_id) === String(currentActive.id)) {
                setActiveChat(prev => {
                    // ✅ FIX: Strict Deduplication using ID or Client-Side ID
                    const existsIndex = prev.messages.findIndex(m =>
                        m.id === newMsg.message_id ||
                        (m.client_side_id && m.client_side_id === newMsg.client_side_id) // Strict match
                    );
    ...
    const handleSendMessage = async (text, file) => {
        ...
        if (file) {
            try {
                ...
                attachmentPreviewUrl = URL.createObjectURL(file); // Local preview
            } catch (err) {
        ...
        const optimisticMsg = {
            id: clientSideId, // Use clientSideId as temp ID
            text: text,
